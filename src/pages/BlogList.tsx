      ...
      <section className="navy-gradient py-16 sm:py-28 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10 text-center">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}>
            <h1 className="font-heading text-3xl sm:text-5xl lg:text-6xl font-bold text-white mb-4">Legal <span className="text-gold-gradient">Blog & Insights</span></h1>
            <p className="text-white/60 text-base sm:text-lg max-w-2xl mx-auto">Expert articles and updates on Pakistani law from Resolution Law Firm</p>
          </motion.div>
        </div>
      </section>

      <section className="py-12 sm:py-28 max-w-7xl mx-auto px-4 sm:px-6">
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
            {[1,2,3].map(i => (
              <div key={i} className="bg-white rounded-xl overflow-hidden animate-pulse">
                <div className="h-44 sm:h-48 bg-cream-dark" />
            ...
            {blogs.map((blog, i) => (
              ...
              >
                <Link to={`/blog/${blog.slug}`} className="block bg-white rounded-xl overflow-hidden border border-cream-dark hover:border-gold/30 hover:shadow-lg transition-all group h-full">
                  <div className="h-44 sm:h-48 bg-navy/5 overflow-hidden">
                    {blog.image_url ? (
                  ...
                  <div className="p-4 sm:p-5">
                    ...
                    <h3 className="font-heading text-base sm:text-lg font-bold text-navy mb-2 group-hover:text-gold transition-colors">{blog.title}</h3>
